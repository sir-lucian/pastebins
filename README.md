# Summary of DJMax Respect V - V Liberty 3 Incident for English speakers 101

Greetings, this is Sir Lucian. Guest Pattern Designer of DJMax Respect V. I'm writing this on my own GitHub page just because Google Docs or posting on any other SNS directly are more prone to outrage attacks, so please bare with me

### Disclaimer
**I'm not writing this on Neowiz, Rocky Studio, or any official groups behalf.** I'm only doing this so that the global side could finally have a better grasp of what had happened from the very beginning, by 3rd party's perspective.

So that I can finally slap people who said "They're just overreacting!" or "This is just gonna destroy your own game" in the face because they cannot comprehend the cultural differences.

With that said, hold your thoughts for a bit, and let us begin.

## Part 1: The involved parties

Before going into the current situation that is happening. We should introduce some entities that will get involved first, so that we are in the same page since I expect that some of you might not know any of these.

First, the good ol' DJMax, the big guy. DJMax is a rhythm game series that has survived for years with multiple teams in charge over a long history. Nowadays, we have DJMax Respect V, a live service game on Steam and Xbox Game Pass developed by Rocky Studio Team led by BEXTER, under Neowiz, a game company based in South Korea.

Next one, we have to start with some context first. Let me introduce you to "Waktaverse", a Vtuber group from South Korea. This group is led by an og content creator/streamer of South Korea "Woowakgood", which is one of the main entities we will discuss soon. Now, as you can see, Waktaverse group is huge, and not everyone is relevant in this story. So for the sake of simplicity, we are going to focus on its specific unit called "Isegye Idol" or "Isedol", an idol unit created to have the main specialty being girl group music and streaming content.

This group also posess a DJMax fan game call WJMax (WakJMax). A fan game that was given a permission by Neowiz to operate as a non-commercial game featuring the songs from Waktaverse itself with its own team managing and content curating.

Now at this point, you've already know all the official parties. Let me introduce "Ipari", a fandom name of Isedol fan group, or for the sake of western pronunciation I'm just going to call it "Leafies." (Ipari is leaf in Korean) 

"But hold on" you might say. "Why are the Leafies being here? Isn't it about corporates' conflict of interest?" I'm glad you asked, because the starting of the descend to the rabbit hole just happened to be because of the fan base that treat idols like demigods, turning into a belief system, followed by endorsement from the idols and managers themselves which led to several catastrophic scandals and controversies that the list on the main wiki page is longer than a single politician's biography.

## Part 2: The prelude

At this point you can already assume that idol fan base can be obnoxious, and that's exactly what happened multiple times throughout history with the Leafies.

Now, some of the more notorious Leafies were known to cause ruckus on other communities by generally being an obnoxious fan like spamming on various online forums, criticising other Vtubers that their idols are better than them, picking pointless fights on other communities such as custom keyboards groups, Blue Archive, Vocaloid, Monster Hunter, and even South Korean politics.

But wait, there's more. There are cases that some Leafies were engaged in active cyberbullying content creators, a cosplayer, other Vtubers even withing Waktaverse itself, and probably more to list that you could imagine. The most tragic is the cyberbullying of Airi Kanna which included doxxing her family member, terrorising her streams, and eventually cyberbully her into her graduation indirectly (citation needed).

As for Woowakgood, he's known to be the archnemesis of DJMax starting the moment when he left a Steam curator review on DJMax back in the days slandering DJMax that the songs are old and boring, when the direction of the game was to respect old players; Hence the name DJMax Respect V.

Needless to say, the public image of this fandom in particular is not very good to begin with.

## Part 3: WJMax

Back in October 2022, Woowakgood held a fan game contest centered around Waktaverse. WJMax was created by a different team for this, initially. At the time, it hasn't been approved and endorsed by Neowiz yet. When the WJMax continued to be developed post contest later, it finally got the approval from Neowiz that "It can be operated with a condition that it will remain non-commercial". Thus the Neowiz credit was inserted.

The most recognisable characteristic of WJMax that was **significantly different than DJMax at the time** was the story mode. (we'll come back to this later) It included the lore and story of Waktaverse members. Specifically Isedol members and their journey.

Over time, it got continuous updates in both features and the songs. However, in addition from Waktaverse original songs, it also included unauthorised covers. Not only that, the game itself is operated by **downloading the unencripted files - including the sound and video sources**, which broke the illegal cloning and distribution laws.

## Part 4: The "Kidding" incident

On 12 June 2025, DJMax Respect V held an official broadcast promoting the new DLC "V Liberty 3". The new song list included "Kidding", an Isedol song. The Korean DJMax community immediately went in flames considering the controversies record of the fandom above. The official discord, Korean general chat in particular, went into war zone, demanding the song to be remove or replaced. Shortly after that, the boycott review bombs begun. It was so bad that the most recent reviews rating went down to "Overwhelmingly Negative".

To global readers, **No, the reviews weren't about "always online DRM" like usual, so hold your salty brain for a second.**

After a long day, on 13 June 2025, DJMax Respect V finally came back from silence (they probably were in an emergency meeting) stating that "Kidding" will finally be replaced. The replaced song will come later (so not on launch day). This has subsided the war zone a lot, but some are still fighting because **there might be another issue lurking in the distance** which we will cover in the later section.

## Part 5: The direct aftermath

Soon after, there were many apology statements by different parties. We'll go through some of the most important one-by-one.

First, Woowakgood's [apology](https://cafe.naver.com/steamindiegame/20339352)

One thing to mention is that, he posted this statement on his own fan blog. It's publicly seen with no subscription payment required, but it's his fan blog which made it look like he's only apologising to his fans. Also the apology is shockingly short and covers no details at all, so that just added the spice that he might not have the remorse at all.

Second, CLTH, the composer of "Kidding", and this is [his statement](https://www.youtube.com/channel/UCUYJ2ZTHq1GKOoam-r_SL1w/community?lb=UgkxTGiDb__I6gAfN8tsLyb37JMNCRjhg4hg).

The most important part said that "Kidding" was included in the DLC not on his own will, meaning Neowiz took his song, made a DJMax edit/remix, and slapped on the DLC without him knowing it's happening. Whether if this is true or not, Neowiz is already responsible for either choosing songs without stakeholders' consent, or having no background checking of the song's controversies by internal PR team.

We don't know if CLTH's claim is absolutely true or not. Is it correct or just him trying to throw Neowiz under the bus it's up to you to believe. Proceed with caution, alright?

And we'll come back to CLTH's another work soon.

Next we have main content creators side. Kim Pyun Jip's (ARPA) and REMILIA's statements [[ARPA]](https://www.youtube.com/channel/UCqIsRcbdqG2AyR4dZWaLAGg/community?lb=UgkxtbREOtb79cAcVuieO9jUVd5btNW67L_5) [[REMILIA]](https://www.youtube.com/channel/UCpeRFpJFnFVjGPFjbe05amQ/community?lb=UgkxjrV1marBh8LJjABjSjHcfPnM6JAuCV3O)

Both of them had some involvement with WJMax in the past and gradually stepped back from making WJMax content after noticing the issues with copyrighted content. Then finally, they donated the revenue they got from WJMax content to other good uses.

At this point, WJMax is now becoming the center attention of this butterfly effect. So as I'm writing this, it will continue to have their issues being addressed further, with multiple lawsuits from other parties that follow. (Too many to list and is not finalised as of writing. It's still an ongoing issue with no definitive conclusion yet, and it's not really relevant for most of us reading this unless you follow Waktaverse as a non Korean fan.)

Furthermore, DJMax later modified the user generated content policies to disallow all fan games using DJMax IP down the line unless Neowiz specify otherwise, and DJMax Respect V will not include existing licensed songs in the main line DLC (Extension and Liberty so far) from now on.

## Part 6: DJMax's long term direction and global side consequences

### Disclaimer 2
This part will include speculations and predictions. None of these situations will be 100% accurate nor is leaked information. If things might seem to connect to each other, it's purely coincidence from outsider perspective at the point of writing this.

Anyway, following the main incident, many noticed that DIEIN's debut song on their channel ([which is confirmed to be a part of LENA Project](https://x.com/LENAENTofficial/status/1933455972468732349)) has credited CLTH by composing and listing vocal as "DIEIN" themself. This opened the discussion topic of who was singing as DIEIN.

Some speculations point at a Waktaverse Vtuber due to having the same vocal range and style, but it's not confirmed by any means. The suspected individual's identity will not be stated in this summary for both sides' safety.

This may raise a theory that maybe Neowiz was trying to use CLTH's connection with Waktaverse to "merge" the universe together with DJMax side to be fully integrated later. The main evidence is that V Liverty 2 DLC added story mode of its own, similar to WJMax. The storyline that is focusing on idols and entertainment. The multiverse theme. If this were to be successful, DJMax would absorb the Waktaverse fandom into its own and we would've seen many more VTuber songs in the future because VTuber industry makes a lot of money (supposedly).

However, if became a total disaster when the fandom level had a unanimous consensus that Waktaverse fandom (Leafies in particular) were not to be messing with.

*Will this incident affect global side?* I'd say **barely matters**. The only thing we will be missing out are great licensed songs that could've been added to please global fans like Monstercat or Attack the Music. But we will see what will happen down the road. Only time will tell.

One more thing to note that, the direcction od DJMax storyline might or might not change depends on how far did they go with the concept they've planned so far. We'll see that in the future.

~~BUT HEY THAT'S JUST A THEORY. A GAME THEORY!~~

Anything else would be less than 1% relevant for most global fans I'd assume. Please, stay safe, stay healthy. Stop gooning and hit buttons. If there are any mistakes or additional information that might elaborate things further, please let me know. Fork this repo and send me merge requests. If you're from Waktaverse's side, please also be civil and cooperative. I'm open to listen on both sides.

Sir Lucian, peace.
18 June 2025
